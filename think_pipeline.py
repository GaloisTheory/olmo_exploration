...
DPO_PARQUET = "hf://datasets/allenai/Dolci-Think-DPO-7B/**/*.parquet"
RL_PARQUET = "hf://datasets/allenai/Dolci-Think-RL-7B/**/*.parquet"
...
# %%
# Show all rows of unique dataset_source values
unique_sources = sft_lf.select(pl.col("dataset_source")).unique().collect()
with pl.Config(tbl_rows=-1):  # Show all rows, not just head
...
# %% Cell B: Source distribution (exact — polars reads only the dataset_source column)
...
for row in sft_source_dist.iter_rows(named=True):
    ...
    bar = "█" * int(pct / 2)
    print(f"  {row['dataset_source']:55s} {row['len']:9,d}  {pct:5.1f}%  {bar}")
print(f"\n  Total rows: {total:,}")

# Bar chart
fig, ax = plt.subplots(figsize=(12, max(4, len(sft_source_dist) * 0.4)))
labels = sft_source_dist["dataset_source"].to_list()
...
ax.set_xlabel("% of dataset (full 2.27M rows)")
...
for source_name in sft_source_dist["dataset_source"]:
    examples = (
        sft_df
        .filter(pl.col("dataset_source") == source_name)
...
# Check for null/empty dataset_source (identity prompts might lack a source tag)
null_source = sft_df.filter(pl.col("dataset_source").is_null() | (pl.col("dataset_source") == ""))
print(f"  Rows with null/empty dataset_source: {len(null_source):,}")

# Paper says 2,268,468 total for 7B; we have {len(sft_df)}. Diff = identity prompts?
EXPECTED_TOTAL = 2_268_468
print(f"  Expected (paper): {EXPECTED_TOTAL:,}")
print(f"  Collected (HF):   {len(sft_df):,}")
print(f"  Missing:          {EXPECTED_TOTAL - len(sft_df):,}  (paper lists 290 OLMo Identity Prompts)")
...
if len(identity_matches) > 0:
    id_sources = identity_matches.group_by("dataset_source").len().sort("len", descending=True)
    for row in id_sources.iter_rows(named=True):
        print(f"    {row['dataset_source']}: {row['len']}")
    for i, row in enumerate(identity_matches.head(3).iter_rows(named=True), 1):
        print(f"\n  Example {i} (source: {row['dataset_source']}):")
        print(f"    {row['content'][:300]}...")
else:
    print("  → Identity prompts are NOT in the published HF parquet files.")
    print("    They were likely mixed in at training time (290 prompts = 58 unique × 5 reps).")

# List HF repo files to confirm
print("\n  HF repo parquet files:")
for f in hf_api.list_repo_files("allenai/Dolci-Think-SFT-7B", repo_type="dataset"):
...
for row in dpo_source_dist.iter_rows(named=True):
    ...
    bar = "█" * int(pct / 2)
    print(f"  {row['dataset_source']:50s} {row['len']:7,d}  {pct:5.1f}%  {bar}")
